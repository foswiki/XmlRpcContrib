...
sub dispatch {
    ...
    if ( $ENV{'CONTENT_TYPE'} ne 'text/xml' ) {
        $this->printError( '415 Unsupported Media Type',
            -32300, 'Only XML-RPC POST requests recognised.' );
        return;
    }
    ...
    unless ( ref($request) ) {
        $this->printError( '400 Bad Request', -32700, $request )
          unless ref($request);
        return;
    }
    ...
    # check impl
    unless ($handler) {
        $this->printError( '501 Not Implemented',
            -32601, "Method $name not supported" );
        return;
    }
...
sub printError {
    ...
    $this->print( $status,
        $this->getError( -32300, 'Only XML-RPC POST requests recognised.' ) );
}
